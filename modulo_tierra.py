# modulo_tierra.py — EcoVolt Pro · Puesta a Tierra
# ─────────────────────────────────────────────────────────────────────────────
"""
...
def generar_pdf_tierra(datos: dict) -> bytes:
    ...
    MGRAY  = colors.HexColor("#0B2545")
    DGRAY  = colors.HexColor("#0B2545")
    BORDER = colors.HexColor("#0B2545")
    BLUE   = colors.HexColor("#0B2545")
    LBLUE  = colors.HexColor("#F8F9FA")
    WHITE  = colors.white
    ...
    story.append(p(f"ECOVOLT PRO — Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}", st_foot))
    story.append(pb())
            ...
            t_med.setStyle(TableStyle([
                ...
                *[("BACKGROUND", (0,i), (-1,i), WHITE if i%2==1 else LGRAY) for i in range(1,n_r-1)],
                ("BACKGROUND",   (0,n_r-1), (-1,n_r-1),
                 colors.HexColor("#F8F9FA") if cumple else colors.HexColor("#F8F9FA")),
    ...
    if rho < 30:
        ...
        color_suelo = colors.HexColor("#FF9F1C")
    else:
    ...
    if all_cumple is True:
        ...
        res_col = colors.HexColor("#FF9F1C")
    ...
    story.append(p(
        f"ECOVOLT PRO  |  Informe Puesta a Tierra  |  Norma LA-400 / RETIE / NTC 2050  |  "
    ...
    story.append(hr(SOL, 1))
    story.append(p(
        f"ECOVOLT PRO  ·  Informe Puesta a Tierra  ·  Norma LA-400 / RETIE  ·  "
        f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}",
        st_foot))
...
def mostrar_tierra(proyecto_id: int, session_state: dict):
    ...
    <div class='hero-header'>
        <div class='hero-title'>⏚ PUESTA A TIERRA
            <span style='font-size:0.5em;background:#0B2545;color:#fff;padding:2px 12px;
    ...
    with tab_dis:
        ...
        with col_d1:
            ...
            <div class='formula-box'>
                Límite RETIE para <b>{tipo_retie}</b>: <b style='color:#FF9F1C;'>R ≤ {r_objetivo:.0f} Ω</b>
            </div>""", unsafe_allow_html=True)
        ...
        with col_d2:
            ...
            <div class='sol-card' style='margin-top:0.6rem;'>
                <div style='color:#0B2545;font-family:Rajdhani,sans-serif;font-weight:700;
                            margin-bottom:0.6rem;'>CÁLCULO (fórmula de Dwight)</div>
                <table style='width:100%;font-size:0.82rem;border-collapse:collapse;'>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.3rem 0;'>ρ efectiva</td>
                        <td style='font-family:Share Tech Mono;text-align:right;'>{rho_ef:.1f} Ω·m</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.3rem 0;'>R₁ varilla (Dwight)</td>
                        <td style='font-family:Share Tech Mono;text-align:right;'>{r1:.2f} Ω</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.3rem 0;'>Varillas mínimas para ≤{r_objetivo:.0f} Ω</td>
                        <td style='font-family:Share Tech Mono;color:#FF9F1C;font-weight:700;text-align:right;'>{n_var} und</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.3rem 0;'>R calculada ({n_varillas} varillas)</td>
                        <td style='font-family:Share Tech Mono;color:{"#2E7D32" if cumple_calc else "#0B2545"};
                                   font-weight:700;text-align:right;'>{r_calc:.2f} Ω</td>
                    </tr>
                    <tr>
                        <td style='color:#2E7D32;padding:0.3rem 0;'>Separación mínima entre varillas</td>
                        <td style='font-family:Share Tech Mono;text-align:right;'>{sep_var:.1f} m</td>
                    </tr>
                </table>
                <div style='margin-top:0.6rem;background:{"#F8F9FA" if cumple_calc else "#F8F9FA"};
                            border-radius:6px;padding:0.5rem;text-align:center;
                            font-weight:700;font-size:0.85rem;
                            color:{"#2E7D32" if cumple_calc else "#0B2545"};'>
    ...
    with tab_cdt:
        ...
        for _hc, _ht in zip(_hcols,
            ...
            _hc.markdown(
                f"<div style='font-size:0.72rem;color:#0B2545;font-weight:700;"
        ...
        if vals_para_prom:
            ...
            color_prom = "#2E7D32" if cumple_r else "#0B2545"
            color_val  = "#0B2545" if es_valido else "#FF9F1C"
    ...
    with tab_wen:
        ...
        for i, d_sug in enumerate(D_SUGERIDAS):
            ...
            with wc[2]:
                ...
                st.markdown(f"""
                <div style='background:#F8F9FA;border-radius:6px;padding:0.42rem 0.5rem;
                            font-family:Share Tech Mono;font-size:0.82rem;
                            color:{"#0B2545" if rho_v else "#0B2545"};text-align:center;
                            margin-top:0.1rem;'>
            ...
            with wc[4]:
                if rho_v:
                    terreno_detectado = next(
                        ...
                        "—")
                    st.markdown(
                        f"<div style='font-size:0.65rem;color:#2E7D32;margin-top:0.4rem;'>"
        ...
        if rho_vals:
            ...
            for _wmc, _wlbl, _wval, _wcol in [
                (w_col1, "ρ PROMEDIO",  f"{rho_prom:.1f} Ω·m", "#FF9F1C"),
                (w_col2, "ρ MÍNIMA",   f"{rho_min:.1f} Ω·m",  "#2E7D32"),
                (w_col3, "ρ MÁXIMA",   f"{rho_max:.1f} Ω·m",  "#0B2545"),
                (w_col4, "VARILLAS RECOMENDADAS", f"{n_var_wen} und", "#0B2545"),
            ]:
                _wmc.markdown(f"""
        ...
        if meds_df.empty and wen_df.empty:
            ...
            if not meds_df.empty:
                ...
                for _, row in meds_df.iterrows():
                    c_col = "#2E7D32" if row["cumple_retie"] else "#0B2545"
                    v_col = "#0B2545" if row["r_valido"]    else "#FF9F1C"
                    st.markdown(f"""
                    <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;
                        ...
                        <div style='display:flex;gap:1.5rem;flex-wrap:wrap;align-items:center;'>
                            <span style='color:#FF9F1C;font-family:Rajdhani,sans-serif;
                                         font-weight:700;'>#{row["id"]} {row["lugar"] or "—"}</span>
                            <span style='color:#2E7D32;font-size:0.8rem;'>Punto: {row["punto_sig"] or "—"}</span>
                            <span style='color:#2E7D32;font-size:0.8rem;'>Fecha: {row["fecha_med"] or "—"}</span>
                            <span style='color:#2E7D32;font-size:0.8rem;'>Terreno: {row["estado_terreno"]}</span>
                ...
                <div style='background:#F8F9FA;border-radius:8px;padding:0.6rem 1rem;
                            margin-top:0.3rem;font-size:0.82rem;color:#2E7D32;'>
                    Total mediciones: <b style='color:#FF9F1C;'>{total_m}</b> |
                    Cumplen RETIE: <b style='color:#2E7D32;'>{cumplen}</b> |
                    No cumplen: <b style='color:#0B2545;'>{total_m - cumplen}</b>
                </div>""", unsafe_allow_html=True)
            ...
            if not wen_df.empty:
                ...
                for _, row in wen_df.iterrows():
                    ...
                    <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;
                                padding:0.8rem 1.2rem;margin-bottom:0.5rem;'>
                        <div style='display:flex;gap:1.5rem;flex-wrap:wrap;align-items:center;'>
                            <span style='color:#0B2545;font-family:Rajdhani,sans-serif;
                                         font-weight:700;'>#{row["id"]} {row["lugar"] or "—"}</span>
                            <span style='color:#2E7D32;font-size:0.8rem;'>Fecha: {row["fecha_med"] or "—"}</span>
                            <span style='color:#2E7D32;font-size:0.8rem;'>Terreno: {row["estado_terreno"]}</span>
                            <span style='font-family:Share Tech Mono;color:#0B2545;font-weight:700;'>
