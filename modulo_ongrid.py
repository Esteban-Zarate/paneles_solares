# modulo_ongrid.py — EcoVolt Pro · Módulo ON-GRID (Sistema Conectado a la Red)
# ─────────────────────────────────────────────────────────────────────────────
"""
...
def render_svg_og(svg_string: str, height: int = 700) -> None:
    ...
    html = (f'<div style="width:100%;border-radius:12px;overflow:hidden;">'
            ...
            f'background:#F8F9FA;border-radius:12px;" alt="Plano ON-GRID"/>'
            f'</div>')
...
def svg_plano_paneles_og(n_paneles: int, pan_serie: int, n_strings: int,
    ...
    C_PANEL = "#FFFFFF"
    C_PANEL_B = "#0B2545"
    C_GRID = "#FF9F1C"   # color especial para red eléctrica
    ...
    panels_svg = ""
    pan_count = 0
    string_colors = [C_PANEL_B, "#FF9F1C", "#2E7D32", "#0B2545", "#2E7D32", "#FF9F1C"]
    ...
    for row_i in range(max_rows):
        for col_i in range(max_cols):
            ...
            # Líneas de células
            for lx in range(1, 4):
                panels_svg += line(px + lx*PW//4, py+2, px + lx*PW//4, py+PH-2, "#0B2545", 0.5)
            for ly in range(1, 3):
                panels_svg += line(px+2, py + ly*PH//3, px+PW-2, py + ly*PH//3, "#0B2545", 0.5)
            # Número de panel
    ...
    info_svg = box(INFO_X, INFO_Y, INFO_W, 20 + len(info_items)*22 + 10, "#FFFFFF", "#0B2545", 8)
    info_svg += box(INFO_X, INFO_Y, INFO_W, 20, "#FF9F1C", "#FF9F1C", 8, 0, 0)
    info_svg += txt(INFO_X + INFO_W//2, INFO_Y + 13, "DATOS ARRAY ON-GRID", 8, "#F8F9FA", "middle", "700", "Rajdhani,sans-serif")
    for k, (lbl, val, col) in enumerate(info_items):
        ...
        if k < len(info_items)-1:
            info_svg += line(INFO_X+6, iy+13, INFO_X+INFO_W-6, iy+13, "#FFFFFF", 0.5)
    ...
    legend_svg = box(leg_x, leg_y, INFO_W, 20 + len(leg_items)*18 + 8, "#FFFFFF", "#0B2545", 6)
    ...
    tb_y = H - 68
    title_svg = f'''
    <rect x="0" y="{tb_y}" width="{W}" height="68" fill="#FFFFFF" stroke="#0B2545" stroke-width="0.8"/>
    ...
    <text x="12" y="{tb_y+45}" font-family="Share Tech Mono,monospace" font-size="8" fill="{C_DIM}">
        Pot. instalada: {pot_inst/1000:.2f} kWp  |  Área: {arr_ancho_m:.2f}m × {arr_alto_m:.2f}m  |  EcoVolt Pro — ON-GRID  |  {datetime.now().strftime("%d/%m/%Y")}  |  Plano N°01</text>
    ...
    <rect x="{W-220}" y="8" width="210" height="44" rx="6" fill="#FFFFFF" stroke="{C_GRID}" stroke-width="1"/>
...
def svg_diagrama_unifilar_og(n_paneles: int, pan_serie: int, n_strings: int,
    ...
    arr_block = f'''
    <g id="array_og">
      {box(ARR_X,ARR_Y,ARR_W,ARR_H,"#FFFFFF","#FF9F1C",8)}
      {box(ARR_X,ARR_Y,ARR_W,22,"#FFFFFF","#FF9F1C",8)}
    ...
    for s in range(min(n_strings, 4)):
        arr_block += box(sp_x, sp_y + s*(sp_h+sp_gap), sp_w, sp_h, "#F8F9FA", C_DC, 2)
    ...
    cb_block = f'''
    <g id="combiner">
      {box(CB_X,CB_Y,CB_W,CB_H,"#FFFFFF","#0B2545",6)}
      {box(CB_X,CB_Y,CB_W,20,"#FFFFFF","#0B2545",6)}
      ...
      {box(CB_X+12,CB_Y+60,CB_W-24,18,"#F8F9FA","#0B2545",3)}
      {txt(CB_X+CB_W//2, CB_Y+72, "Fusibles DC Array", 7, "#0B2545")}
      {txt(CB_X+CB_W//2, CB_Y+82, f"≥{i_array:.0f}A", 6.5, C_DIM,"middle","normal","Share Tech Mono,monospace")}
      {box(CB_X+12,CB_Y+84,CB_W-24,18,"#F8F9FA","#2E7D32",3)}
    ...
    inv_block = f'''
    <g id="inversor_og">
      {box(INV_X,INV_Y,INV_W,INV_H,"#FFFFFF","#2E7D32",8,1.5)}
      {box(INV_X,INV_Y,INV_W,22,"#FFFFFF","#2E7D32",8)}
    ...
    med_block = f'''
    <g id="medidor">
      {box(MED_X,MED_Y,MED_W,MED_H,"#FFFFFF","#FF9F1C",6)}
      {box(MED_X,MED_Y,MED_W,20,"#FFFFFF","#FF9F1C",6)}
    ...
    tac_block = f'''
    <g id="tablero_ac">
      {box(TAC_X,TAC_Y,TAC_W,TAC_H,"#FFFFFF","#2E7D32",6)}
      {box(TAC_X,TAC_Y,TAC_W,20,"#FFFFFF","#2E7D32",6)}
      {txt(TAC_X+TAC_W//2, TAC_Y+13, "TABLERO AC", 7.5, C_AC,"middle","700","Rajdhani,sans-serif")}
      {box(TAC_X+10,TAC_Y+28,TAC_W-20,18,"#F8F9FA","#2E7D32",3)}
      {txt(TAC_X+TAC_W//2, TAC_Y+40, "Interruptor gral.", 7, C_AC)}
      {box(TAC_X+10,TAC_Y+50,TAC_W-20,18,"#F8F9FA","#0B2545",3)}
      {txt(TAC_X+TAC_W//2, TAC_Y+62, "Prot. diferencial", 7, "#0B2545")}
      {box(TAC_X+10,TAC_Y+72,TAC_W-20,18,"#F8F9FA","#2E7D32",3)}
    ...
    if cargas_df is not None and not cargas_df.empty:
        ...
        for _li7, (_, _lr7) in enumerate(_sample7.iterrows()):
            ...
            cargas_items_og += line(CGX+8, _ly7+14, CGX+CGW-8, _ly7+14, "#F8F9FA", 0.6)
    ...
    red_block = f'''
    <g id="red_electrica">
      {box(RED_X,RED_Y,RED_W,RED_H,"#FFFFFF","#FF9F1C",8,1.5)}
      {box(RED_X,RED_Y,RED_W,22,"#FF9F1C","#FF9F1C",8)}
      {txt(RED_X+RED_W//2, RED_Y+14, "RED ELÉCTRICA", 8, "#F8F9FA","middle","700","Rajdhani,sans-serif")}
    ...
    <g id="protecciones_og">
      {box(PR_X,PR_Y,PR_W,PR_H,"#FFFFFF","#2E7D32",6)}
      {box(PR_X,PR_Y,PR_W,20,"#FFFFFF","#2E7D32",6)}
      {txt(PR_X+PR_W//2, PR_Y+13, "PROTECCIONES", 7.5, C_DIM,"middle","700")}
      {box(PR_X+8,PR_Y+28,PR_W//2-12,18,"#F8F9FA","#0B2545",3)}
      {txt(PR_X+PR_W//4, PR_Y+40, "Fusible DC", 7, "#0B2545")}
      {box(PR_X+PR_W//2+4,PR_Y+28,PR_W//2-12,18,"#F8F9FA","#2E7D32",3)}
      {txt(PR_X+PR_W*3//4+2, PR_Y+40, "Interr. AC", 7, C_AC)}
      {box(PR_X+8,PR_Y+52,PR_W//2-12,18,"#F8F9FA","#0B2545",3)}
      {txt(PR_X+PR_W//4, PR_Y+64, "SPD DC", 7, "#0B2545")}
      {box(PR_X+PR_W//2+4,PR_Y+52,PR_W//2-12,18,"#F8F9FA","#0B2545",3)}
      {txt(PR_X+PR_W*3//4+2, PR_Y+64, "SPD AC", 7, "#0B2545")}
      {box(PR_X+8,PR_Y+76,PR_W-16,18,"#F8F9FA","#2E7D32",3)}
      {txt(PR_X+PR_W//2, PR_Y+88, "Puesta a tierra TT/TN", 7, C_DIM)}
      {box(PR_X+8,PR_Y+100,PR_W-16,18,"#F8F9FA","#FF9F1C",3)}
    ...
    legend = f'''
    <g>
      {box(leg_x,leg_y,160,120,"#FFFFFF","#0B2545",6)}
    ...
    <text x="12" y="{tb_y+45}" font-family="Share Tech Mono,monospace" font-size="8" fill="{C_DIM}">
        Array: {n_paneles}×{pot_panel}Wp = {n_paneles*pot_panel/1000:.2f}kWp  |  Vmpp={v_array:.0f}V  |  Iarray={i_array:.1f}A  |  EcoVolt Pro ON-GRID  |  {datetime.now().strftime("%d/%m/%Y")}  |  Plano N°02</text>
...
def generar_pdf_ongrid(proyecto_id: int, proyecto_info: tuple, datos: dict) -> bytes:
    ...
    story = []
    story.append(Paragraph("☀  ECOVOLT PRO — DIMENSIONAMIENTO ON-GRID", titulo_st))
...
def generar_pdf_economico_ongrid(proyecto_id: int, proyecto_info: tuple, datos: dict) -> bytes:
    ...
    story = []
    story.append(Paragraph("💹  ECOVOLT PRO — ANÁLISIS ECONÓMICO Y AMBIENTAL (ON-GRID)", titulo_st))
    story.append(Paragraph(
        f"Proyecto: <b>{proyecto_info[1]}</b>  |  Municipio: {proyecto_info[2] or '—'}  |  "
        f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}", sub_st))
    story.append(HRFlowable(width="100%", thickness=1, color=SOL, spaceAfter=8))
...
def mostrar_ongrid(proyecto_id: int, session_state: dict) -> None:
    ...
    <style>
    .og-badge {
        display:inline-block;background:#FF9F1C;color:#F8F9FA;
    ...
    <div class='hero-header'>
        <div class='hero-title'>☀ ECOVOLT PRO <span class='og-badge'>ON-GRID</span></div>
    ...
    with tab_og1:
        ...
        for c, val, unit, lbl, col in [
            ...
            (col3, consumo_base, "Wh/día", "Consumo seleccionado", "#2E7D32"),
            (col4, consumo_fs_og, "Wh/día", "Con 20% FS sistema", "#FF9F1C"),
        ]:
            with c:
                st.markdown(f"""
        ...
        if _horas_aut_og > 0 and "Recibo" in fuente:
            ...
            <div class='info-note' style='margin-top:0.5rem;border-color:rgba(0,188,212,0.3);
                ...
                🔋 Autonomía configurada en Módulo 2: <b style='color:#0B2545;'>
    ...
    with tab_og2:
        ...
        with col_hsp1:
            ...
            with st.expander("📋 Otras fuentes de irradiación", expanded=False):
                ...
                <div style='font-size:0.83rem;color:#2E7D32;line-height:1.9;'>
                    ...
                    <b style='color:#FF9F1C;'>💡 Para ON-GRID:</b>
                    <span style='color:#2E7D32;'>Usar promedio anual.
        ...
        with col_hsp2:
            ...
            <div class='result-highlight'>
                ...
                <div style='color:#0B2545;font-size:0.82rem;margin-top:0.2rem;'>
    ...
    with tab_og3:
            ...
            for _oi, (_pnom, _pp) in enumerate(CATALOGO_PANELES_OG.items()):
                with _cat_cols_og[_oi % 3]:
                    ...
                    <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:8px;
                        ...
                        <div style='font-size:0.72rem;color:#FF9F1C;font-weight:600;'>{_pnom}</div>
                        <div style='font-family:Share Tech Mono;font-size:0.85rem;color:#0B2545;'>{_pp['wp']}Wp</div>
                        <div style='font-size:0.7rem;color:#2E7D32;'>Voc={_pp['voc']}V Vmpp={_pp['vmpp']}V Isc={_pp['isc']}A</div>
                    </div>""", unsafe_allow_html=True)
        ...
        with col_p1:
            ...
            if og_vmpp > 0:
                ...
                st.markdown(f"""
                <div class='formula-box'>
                    Serie min: {_ser_min_og}  |  Serie max: {_ser_max_og}  |  Optimo: <b style='color:#FF9F1C;'>{_ser_opt_og} paneles/string</b><br>
            ...
            <div class='sol-card'>
                ...
                <div style='display:grid;grid-template-columns:1fr 1fr;gap:0.8rem;'>
                    <div style='background:#F8F9FA;border-radius:8px;padding:1rem;text-align:center;'>
                        <div style='font-family:Share Tech Mono;font-size:1.8rem;color:#FF9F1C;'>{og_wp}</div>
                        <div style='font-size:0.75rem;color:#2E7D32;margin-top:0.3rem;'>Wp - POTENCIA PICO</div>
                    </div>
                    <div style='background:#F8F9FA;border-radius:8px;padding:1rem;text-align:center;'>
                        <div style='font-family:Share Tech Mono;font-size:1.8rem;color:#0B2545;'>{og_voc}</div>
                        <div style='font-size:0.75rem;color:#2E7D32;margin-top:0.3rem;'>V - Voc</div>
                    </div>
                    <div style='background:#F8F9FA;border-radius:8px;padding:1rem;text-align:center;'>
                        <div style='font-family:Share Tech Mono;font-size:1.8rem;color:#0B2545;'>{og_vmpp}</div>
                        <div style='font-size:0.75rem;color:#2E7D32;margin-top:0.3rem;'>V - Vmpp</div>
                    </div>
                    <div style='background:#F8F9FA;border-radius:8px;padding:1rem;text-align:center;'>
                        <div style='font-family:Share Tech Mono;font-size:1.8rem;color:#2E7D32;'>{og_impp:.1f}</div>
            ...
            <div class='sol-card' style='margin-top:0.6rem;'>
                ...
                <table style='width:100%;font-size:0.8rem;border-collapse:collapse;'>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.3rem 0;'>Breaker AC</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>
                            <b style='color:#2E7D32;'>Breaker 2P {_breaker_ac_std_og}A</b></td>
                    </tr>
                    <tr>
                        <td style='color:#2E7D32;padding:0.3rem 0;'>DPS AC</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>Tipo II - 275V - 40kA</td>
                    </tr>
                </table>
            </div>""", unsafe_allow_html=True)
    ...
    with tab_og4:
        ...
        with col_d1:
            ...
            st.markdown("<small style='color:#2E7D32;'>Rango MPPT del inversor:</small>",
                        unsafe_allow_html=True)
        ...
        with col_d2:
            ...
            <div class='sol-card' style='border-color:rgba(255,107,53,0.4);margin-bottom:0.8rem;'>
                <div style='color:#FF9F1C;font-family:Rajdhani,sans-serif;font-weight:600;margin-bottom:0.6rem;'>PASO A PASO - PDF RETIE | SISTEMA ON-GRID</div>
                <table style='width:100%;font-size:0.83rem;border-collapse:collapse;'>
                    <tr style='border-bottom:1px solid #0B2545;background:#FFFFFF;'>
                        <td style='color:#FF9F1C;padding:0.35rem 0;font-weight:600;'>1. Ed - Consumo base</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;font-weight:700;'>{consumo_input:,.0f} Wh/dia</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;background:#FFFFFF;'>
                        <td style='color:#FF9F1C;padding:0.35rem 0;font-weight:600;'>2. E_real = Ed / FP ({pr_input}%)</td>
                        <td style='font-family:Share Tech Mono;color:#FF9F1C;text-align:right;font-weight:700;'>{res_og["e_real_wh"]:,.0f} Wh/dia</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.35rem 0;'>3. P_FV = E_real / HSP ({hsp_input}h)</td>
                        <td style='font-family:Share Tech Mono;color:#FF9F1C;text-align:right;font-weight:700;'>{res_og["pfv_w"]:,.0f} W = {res_og["pfv_kw"]:.2f} kWp</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.35rem 0;'>4. N = ceil / {wp_input}Wp + {sobredim_og}% sobredim</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{res_og["n_min"]} -> {n_pan} paneles</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;background:#FFFFFF;'>
                        <td style='color:#FF9F1C;padding:0.35rem 0;font-weight:600;'>5. Prod. panel ({wp_input}x{hsp_input}xFP)</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;font-weight:700;'>{res_og["e_panel_wh"]:,.0f} Wh/dia</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.35rem 0;'>Config. strings ({pan_serie}S x {n_strings}P)</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;font-weight:700;'>{n_pan_real} paneles{f" (+{n_pan_real-n_pan})" if n_pan_real != n_pan else ""}</td>
                    </tr>
                    <tr>
                        <td style='color:#2E7D32;padding:0.35rem 0;'>Pot. real instalada</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;font-weight:700;'>{pot_inst_real/1000:.2f} kWp</td>
                    </tr>
                </table>
            </div>
            ...
            <div class='sol-card' style='margin-top:1rem;'>
                ...
                <table style='width:100%;font-size:0.83rem;border-collapse:collapse;'>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Paneles en serie / string</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{pan_serie}</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Strings en paralelo</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{n_strings}</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Vmpp string</td>
                        <td style='font-family:Share Tech Mono;color:#FF9F1C;text-align:right;'>{v_string_mpp:.1f} V</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Voc string (máx. riesgo)</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{v_string_oc:.1f} V</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Corriente total array</td>
                        <td style='font-family:Share Tech Mono;color:#FF9F1C;text-align:right;'>{i_array:.1f} A</td>
                    </tr>
                    <tr>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Dentro rango MPPT</td>
                        <td style='font-family:Share Tech Mono;text-align:right;
                             color:{"#2E7D32" if v_mppt_min <= v_string_mpp <= v_mppt_max else "#0B2545"};'>
            ...
            def _oog(ok): return "#2E7D32" if ok else "#0B2545"
            ...
            <table style='width:100%;font-size:0.8rem;border-collapse:collapse;background:#FFFFFF;border-radius:8px;'>
                <tr style='border-bottom:1px solid #0B2545;'>
                    <td style='color:#2E7D32;padding:0.35rem 0.5rem;'>① I_ctrl ≈ P_FV / V_mppt_min × 1.25</td>
                ...
                <tr style='border-bottom:1px solid #0B2545;'>
                    <td style='color:#2E7D32;padding:0.35rem 0.5rem;'>② Voc string corregida (Tmin={_temp_min_og}°C)</td>
                    <td style='font-family:Share Tech Mono;color:#0B2545;padding:0.35rem;'>
                ...
                <tr style='border-bottom:1px solid #0B2545;'>
                    <td style='color:#2E7D32;padding:0.35rem 0.5rem;'>③ Corriente array (Impp × N_strings)</td>
                    <td style='font-family:Share Tech Mono;color:#0B2545;padding:0.35rem;'>
                        {impp_input}A × {n_strings} strings = {i_array:.1f}A</td>
                    <td style='font-family:Share Tech Mono;color:#0B2545;padding:0.35rem;'>Verificar vs. máx. inv.</td>
                </tr>
                <tr style='border-bottom:1px solid #0B2545;'>
                    <td style='color:#2E7D32;padding:0.35rem 0.5rem;'>④ Vmpp string en rango MPPT</td>
                    <td style='font-family:Share Tech Mono;color:#0B2545;padding:0.35rem;'>
                ...
                <tr style='border-bottom:1px solid #0B2545;'>
                    <td style='color:#2E7D32;padding:0.35rem 0.5rem;'>⑤⑥ Tipo / N° inversores</td>
                ...
                <tr>
                    <td style='color:#2E7D32;padding:0.35rem 0.5rem;'>⑦ Inversor recomendado</td>
                    <td style='font-family:Share Tech Mono;color:#2E7D32;padding:0.35rem;font-weight:700;' colspan='2'>
            ...
            cob_color     = "#2E7D32" if cobertura_pct >= 100 else ("#FF9F1C" if cobertura_pct >= 80 else "#0B2545")

            # Pre-construir filas condicionales FUERA del f-string
            _row_exc = (
                f'<tr style="border-bottom:1px solid #0B2545;">'
                f'<td style="color:#2E7D32;padding:0.4rem 0;">Excedente inyección red</td>'
                f'<td style="font-family:Share Tech Mono;color:#0B2545;text-align:right;">'
                f'{excedente_mes:.1f} kWh/mes</td></tr>'
            ) if excedente_mes > 0 else ""

            _row_def = (
                f'<tr><td style="color:#2E7D32;padding:0.4rem 0;">Déficit cubierto por red</td>'
                f'<td style="font-family:Share Tech Mono;color:#0B2545;text-align:right;">'
                f'{deficit_mes:.1f} kWh/mes</td></tr>'
            ) if deficit_mes > 0 else ""

            _badge_tail = (
                f'— inyecta <b style="color:#0B2545;">{excedente_mes:.1f} kWh/mes</b> a la red.'
                if excedente_mes > 0 else
                f'— la red aporta <b style="color:#0B2545;">{deficit_mes:.1f} kWh/mes</b>.'
            )
            _badge_icon = (
                '<b style="color:#2E7D32;">✅ Cubre completamente</b>'
                if cobertura_pct >= 100 else
                '<b style="color:#FF9F1C;">⚠ Cubre parcialmente</b>'
            )

            st.markdown(f"""
            <div class='sol-card' style='margin-top:1rem;border-color:rgba(0,230,118,0.4);'>
                <div style='color:#2E7D32;font-family:Rajdhani,sans-serif;font-weight:600;
                            margin-bottom:0.8rem;'>⚡ PRODUCCIÓN ESTIMADA</div>
                <div style='background:#0B2545;border-radius:6px;padding:0.6rem 0.8rem;
                            margin-bottom:0.8rem;font-size:0.82rem;color:#2E7D32;'>
                    <b style='color:#0B2545;'>{pot_inst_real/1000:.2f} kWp</b>
                    &nbsp;×&nbsp;<b style='color:#0B2545;'>{hsp_input} HSP</b>
                    &nbsp;×&nbsp;<b style='color:#0B2545;'>{pr_dec:.2f} PR</b>
                    &nbsp;=&nbsp;<b style='color:#2E7D32;font-size:0.95rem;'>{gen_dia_og4:.2f} kWh/día</b>
                    &nbsp;&nbsp;|&nbsp;&nbsp;
                    ×30 =&nbsp;<b style='color:#2E7D32;font-size:0.95rem;'>{gen_mes_og4:.1f} kWh/mes</b>
                </div>
                <table style='width:100%;font-size:0.85rem;border-collapse:collapse;'>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Generación diaria</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;
                                   font-weight:700;'>{gen_dia_og4:.2f} kWh/día</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Generación mensual</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;
                                   font-weight:700;'>{gen_mes_og4:.1f} kWh/mes</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Generación anual</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;'>{gen_anio_og4:.0f} kWh/año</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;'>Consumo mensual</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{consumo_mes:.1f} kWh/mes</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;background:#FFFFFF;'>
                        <td style='color:#2E7D32;padding:0.4rem 0;font-weight:600;'>Cobertura</td>
                ...
                <div style='margin-top:0.7rem;padding:0.5rem 0.8rem;border-radius:6px;
                    ...
                    {_badge_icon} los <b style='color:#0B2545;'>{consumo_mes:.1f} kWh/mes</b>
                    {_badge_tail}
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            try:
                ...
                st.download_button(
                    ...
                    file_name=f"EcoVolt_ONGRID_{_proy_nombre.replace(' ','_')}_"
    ...
    with tab_og5:
        ...
        if pot_inst_og2 == 0 or consumo_og2 == 0:
            ...
            <div class='info-note' style='margin-bottom:0.8rem;'>
                ...
                Gen.: <b style='color:#2E7D32;'>{gen_dia:.2f} kWh/día</b> |
                Autoconsumo: <b style='color:#FF9F1C;'>{autoconsumo_pct:.0f}%</b> |
                Inyección red: <b style='color:#0B2545;'>{inyeccion:.2f} kWh/día</b>
            </div>""", unsafe_allow_html=True)
            ...
            with col_e2:
                ...
                _pb_co  = "#2E7D32" if payback<=7 else "#FF9F1C" if payback<=12 else "#0B2545"
                _vn_co  = "#2E7D32" if vpn_final_og>0 else "#0B2545"
                _ti_co  = "#2E7D32" if tir_og>tmar_og else "#0B2545"
                ...
                <div class='result-highlight' style='border-color:rgba(255,107,53,0.5);'>
                    ...
                    <div class='val' style='color:#FF9F1C;'>${beneficio_mes:,.0f} / mes</div>
                </div>
                <div class='metric-grid' style='margin-top:0.8rem;'>
                    ...
                    <div class='metric-box' style='border-color:rgba(0,230,118,0.5);'>
                        <div class='metric-val' style='color:#2E7D32;'>${beneficio_anio/1e6:.2f}M</div>
                    ...
                    <div class='metric-box'>
                        <div class='metric-val' style='color:#0B2545;'>{pb_desc_og}</div>
                ...
                <div class='sol-card' style='margin-top:0.8rem;'>
                    ...
                    <table style='width:100%;font-size:0.82rem;border-collapse:collapse;'>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>{n_pan_og2} × Panel {wp_og2}Wp</td><td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>${inv_paneles:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>Inversor ON-GRID {pot_inv_og2:.1f}kW</td><td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>${inv_inv:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>Cableado + protecciones</td><td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>${pcable_og:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>Estructura + MO</td><td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>${otros_costos:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;background:#FFFFFF;'><td style='color:#FF9F1C;font-weight:600;'>INVERSIÓN TOTAL</td><td style='font-family:Share Tech Mono;color:#FF9F1C;text-align:right;font-weight:700;'>${inv_total:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>Ahorro autoconsumo/mes ({autoconsumo_pct:.0f}%)</td><td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;'>${ahorro_mes:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>Ingreso inyección/mes</td><td style='font-family:Share Tech Mono;color:#FF9F1C;text-align:right;'>${ing_iny_mes:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;background:#FFFFFF;'><td style='color:#2E7D32;font-weight:600;'>BENEFICIO MENSUAL</td><td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;font-weight:700;'>${beneficio_mes:,.0f}</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>Gen. anual (año 1)</td><td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{gen_anio:,.0f} kWh</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>CO₂ evitado/año</td><td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{co2_anio:.0f} kg</td></tr>
                        <tr style='border-bottom:1px solid #0B2545;'><td style='color:#2E7D32;'>Árboles equiv.</td><td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>≈{arboles_og:.0f}/año</td></tr>
                        <tr><td style='color:#2E7D32;'>TIR</td><td style='font-family:Share Tech Mono;text-align:right;color:{_ti_co};font-weight:700;'>{tir_og:.1f}% {"✅>TMAR" if tir_og>tmar_og else "⚠<TMAR"}</td></tr>
                    </table>
                </div>""", unsafe_allow_html=True)
            ...
            try:
                ...
                st.download_button(
                    ...
                    file_name=f"EcoVolt_Economico_ONGRID_{_proy_nombre.replace(' ','_')}_"
                              f"{datetime.now().strftime('%Y%m%d')}.pdf",
    ...
    with tab_og6:
        ...
        if n_pan_p6 == 0:
            ...
            with cols_leg[0]:
                st.markdown(f"""
                <div class='sol-card'>
                    <div style='font-family:Rajdhani,sans-serif;color:#FF9F1C;font-weight:600;margin-bottom:0.6rem;'>
                    ARRAY ON-GRID</div>
                    <div style='font-size:0.82rem;line-height:1.9;'>
                        🔆 Paneles totales: <b style='color:#0B2545;'>{n_pan_p6}</b><br>
                        📐 Configuración: <b style='color:#0B2545;'>{pan_s_p6}S × {n_str_p6}P</b><br>
                        ⚡ Potencia panel: <b style='color:#0B2545;'>{wp_p6} Wp</b><br>
                        🏭 Pot. instalada: <b style='color:#0B2545;'>{n_pan_p6*wp_p6/1000:.2f} kWp</b>
                    </div>
                </div>""", unsafe_allow_html=True)
            with cols_leg[1]:
                ...
                <div class='sol-card'>
                    ...
                    <div style='font-size:0.82rem;line-height:1.9;font-family:Share Tech Mono,monospace;'>
                        Columnas: <b style='color:#0B2545;'>{arr_cols} paneles</b><br>
                        Filas: <b style='color:#0B2545;'>{arr_rows} strings</b><br>
                        Ancho: <b style='color:#0B2545;'>≈{arr_cols*1.134:.2f} m</b><br>
                        Alto: <b style='color:#0B2545;'>≈{arr_rows*0.686:.2f} m</b><br>
                        Área: <b style='color:#2E7D32;'>≈{arr_cols*1.134*arr_rows*0.686:.1f} m²</b>
                    </div>
                </div>""", unsafe_allow_html=True)
            with cols_leg[2]:
                ...
                <div class='sol-card'>
                    ...
                    ELÉCTRICO</div>
                    <div style='font-size:0.82rem;line-height:1.9;font-family:Share Tech Mono,monospace;'>
                        Vmpp string: <b style='color:#0B2545;'>{v_str:.0f} V</b><br>
                        Iarray total: <b style='color:#0B2545;'>{i_arr:.1f} A</b><br>
                        Tipo: <b style='color:#FF9F1C;'>ON-GRID / Grid-Tie</b><br>
                        Norm: <b style='color:#2E7D32;'>NTC 2050 / RETIE</b>
                    </div>
                </div>""", unsafe_allow_html=True)
        ...
        if n_pan_p7 == 0:
            ...
            with cols7[0]:
                ...
                <div class='sol-card'>
                    ...
                    <div style='font-size:0.82rem;line-height:1.9;'>
                        🔆 Array FV: <b style='color:#0B2545;'>{n_pan_p7} × {wp_p7}Wp</b><br>
                        🔌 Caja combiner DC: <b style='color:#0B2545;'>{n_str_p7} strings</b><br>
                        🔄 Inversor grid-tie: <b style='color:#0B2545;'>{pot_inv_p7:.1f} kW</b><br>
                        📟 Medidor bidireccional: <b style='color:#FF9F1C;'>CREG 030-2018</b><br>
                        🛡 Protecciones: <b style='color:#0B2545;'>DC+AC+SPD+Tierra</b>
                    </div>
                </div>""", unsafe_allow_html=True)
            with cols7[1]:
                st.markdown(f"""
                <div class='sol-card'>
                    ...
                    <div style='font-size:0.82rem;line-height:1.9;font-family:Share Tech Mono,monospace;'>
                        Vmpp string: <b style='color:#0B2545;'>{v_str_p7:.0f} V DC</b><br>
                        Voc string: <b style='color:#0B2545;'>{v_oc_p7:.0f} V DC</b><br>
                        Corriente array: <b style='color:#0B2545;'>{i_arr_p7:.1f} A</b><br>
                        Salida inversor: <b style='color:#2E7D32;'>220V AC 60Hz</b>
                    </div>
                </div>""", unsafe_allow_html=True)
            with cols7[2]:
                ...
                <div class='sol-card'>
                    ...
                    INDICADORES</div>
                    <div style='font-size:0.82rem;line-height:1.9;font-family:Share Tech Mono,monospace;'>
                        Gen. anual: <b style='color:#0B2545;'>{gen_anio_p7:,.0f} kWh</b><br>
                        CO₂ evitado: <b style='color:#2E7D32;'>{co2_p7:,.0f} kg/año</b><br>
                        Payback: <b style='color:#FF9F1C;'>{payback_p7:.1f} años</b><br>
                        HSP: <b style='color:#0B2545;'>{hsp_p7:.2f} h/día</b>
                    </div>
                </div>""", unsafe_allow_html=True)
    ...
    <div style='text-align:center;padding:2rem 0 1rem;color:#0B2545;font-size:0.75rem;letter-spacing:2px;'>
        ECOVOLT PRO · DIMENSIONAMIENTO FOTOVOLTAICO ON-GRID · CREG 030-2018 · RETIE · NTC 2050
    </div>""", unsafe_allow_html=True)
