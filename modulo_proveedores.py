...
def _iframe_html(url: str, height: int = 700) -> str:
    ...
    <div style='width:100%;border-radius:12px;overflow:hidden;
                border:1px solid #0B2545;background:#F8F9FA;'>
        <div style='background:#FFFFFF;padding:0.5rem 1rem;font-size:0.75rem;
                    color:#2E7D32;font-family:Share Tech Mono,monospace;
                    display:flex;align-items:center;gap:0.5rem;'>
            <span style='width:10px;height:10px;border-radius:50%;
                         background:#0B2545;display:inline-block;'></span>
            <span style='width:10px;height:10px;border-radius:50%;
                         background:#FF9F1C;display:inline-block;'></span>
            <span style='width:10px;height:10px;border-radius:50%;
                         background:#2E7D32;display:inline-block;'></span>
...
def mostrar_proveedores():
    ...
    <div style='background:linear-gradient(135deg,#F8F9FA,#FFFFFF);
                border:1px solid #0B2545;border-radius:12px;
                padding:1.2rem 1.5rem;margin-bottom:1.5rem;'>
        <div style='font-family:Rajdhani,sans-serif;font-size:1.6rem;
                    font-weight:700;color:#FF9F1C;letter-spacing:2px;'>
            🏪 PROVEEDORES Y CONSULTA DE PRECIOS</div>
        <div style='color:#2E7D32;font-size:0.8rem;letter-spacing:2px;margin-top:0.2rem;'>
    ...
    <div style='font-family:Rajdhani,sans-serif;font-size:1.1rem;font-weight:700;
                color:#FF9F1C;letter-spacing:1px;margin-bottom:1rem;'>
    ...
    for i, prov in enumerate(co_provs):
        with cols_co[i]:
            st.markdown(f"""
            <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:12px;
                ...
                <div style='font-size:0.75rem;color:#2E7D32;margin-bottom:0.5rem;'>
                    📍 {prov["ciudad"]}</div>
                <div style='font-size:0.78rem;color:#0B2545;line-height:1.5;margin-bottom:0.6rem;'>
                    {prov["descripcion"]}</div>
                <div style='font-size:0.72rem;color:#2E7D32;'>
                    <b style='color:#0B2545;'>Marcas:</b> {prov["marcas"]}<br>
                    <b style='color:#0B2545;'>Tipo:</b> {prov["tipo"]}<br>
                    <b style='color:#0B2545;'>Moneda:</b> {prov["moneda"]}
                </div>
            </div>""", unsafe_allow_html=True)
    ...
    # ── Tabs por proveedor ───────────────────────────────────────────────────
    st.markdown("<hr style='border-color:#0B2545;margin:2rem 0 1rem;'>", unsafe_allow_html=True)
    st.markdown("""
    <div style='font-family:Rajdhani,sans-serif;font-size:1.1rem;font-weight:700;
                color:#FF9F1C;letter-spacing:1px;margin-bottom:1rem;'>
    ...
    for tab, prov in zip(tabs, PROVEEDORES):
        with tab:
            ...
            st.markdown(f"""
            <div style='background:#FFFFFF;border:1px solid {prov["logo_color"]}33;
                ...
                <div>
                    ...
                    <div style='font-size:0.78rem;color:#2E7D32;margin-top:0.3rem;'>
                        {prov["contacto"]}</div>
                    <div style='font-size:0.75rem;color:#0B2545;margin-top:0.2rem;'>
                        {prov["tipo"]}  ·  Moneda: {prov["moneda"]}</div>
                </div>
                <a href="{prov["url"]}" target="_blank"
                   style='background:{prov["logo_color"]};color:#F8F9FA;
            ...
            st.markdown(f"""
            <div style='font-size:0.78rem;color:#2E7D32;margin-bottom:0.6rem;'>
                ...
                with cat_cols[ci % 3]:
                    ...
                    <a href="{cat_url}" target="_blank"
                       style='display:block;background:#F8F9FA;
                              border:1px solid #0B2545;border-radius:6px;
                              padding:0.5rem 0.8rem;margin-bottom:0.4rem;
                              color:#0B2545;text-decoration:none;font-size:0.78rem;
    ...
    # ── Notas de uso ──────────────────────────────────────────────────────────
    st.markdown("""
    <hr style='border-color:#0B2545;'>
    <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;
                padding:1rem 1.2rem;font-size:0.8rem;color:#2E7D32;line-height:1.8;'>
        <b style='color:#FF9F1C;font-family:Rajdhani,sans-serif;'>📌 NOTAS IMPORTANTES</b><br>
