...
def _color_cdt(cumple: bool, pct: float, limite: float) -> str:
    if not cumple:       return "#0B2545"
    if pct > limite*0.8: return "#FF9F1C"
    return "#2E7D32"

def _criterio_badge(crit: str) -> str:
    if crit == "Caída de tensión":
        return "<span style='background:#0B2545;color:#0B2545;padding:2px 7px;border-radius:4px;font-size:0.72rem;'>ΔV</span>"
    return "<span style='background:#2E7D32;color:#2E7D32;padding:2px 7px;border-radius:4px;font-size:0.72rem;'>AMP</span>"
...
def generar_pdf_cableado(tramos: list, params: dict,
    ...
    WARN  = colors.HexColor("#FF9F1C")
    PURP  = colors.HexColor("#2E7D32")
    BORD  = colors.HexColor("#0B2545")
    WHITE = colors.white
    ...
    # ── Portada / Encabezado ──────────────────────────────────────────────────
    story.append(Paragraph("ECOVOLT PRO — MEMORIA TECNICA DE CABLEADO FV", titulo_st))
    ...
    notas = [
        ...
        f"• Generado por ECOVOLT PRO — {datetime.now().strftime('%d/%m/%Y %H:%M')}",
    ]
    for n in notas:
...
def mostrar_cableado(proyecto_id: int, session_state: dict) -> None:
    ...
    <div style='font-family:Rajdhani,sans-serif;font-size:1.5rem;font-weight:700;
                color:#FF9F1C;letter-spacing:2px;margin-bottom:0.5rem;'>
    ...
    <div style='font-family:Rajdhani,sans-serif;font-size:1.15rem;font-weight:700;
                color:#0B2545;letter-spacing:1px;margin-bottom:0.8rem;'>
    ...
    <div style='display:grid;grid-template-columns:2.5fr 0.8fr 0.8fr 0.8fr 0.8fr 0.8fr 0.9fr 0.9fr 1fr;
                gap:2px;background:#0B2545;border-radius:8px 8px 0 0;
                padding:0.5rem 0.8rem;font-size:0.72rem;color:#2E7D32;
    ...
    for i, t in enumerate(tramos):
        bg = "#FFFFFF" if i % 2 == 0 else "#0B2545"
        ...
        <div style='display:grid;grid-template-columns:2.5fr 0.8fr 0.8fr 0.8fr 0.8fr 0.8fr 0.9fr 0.9fr 1fr;
            ...
            <span style='color:#0B2545;font-weight:500;'>{t["nombre"]} {criterio}</span>
            <span style='font-family:Share Tech Mono;color:#0B2545;'>{t["corriente_a"]:.1f}</span>
            <span style='font-family:Share Tech Mono;color:#FF9F1C;'>{t["i_dis"]:.1f}</span>
            <span style='font-family:Share Tech Mono;color:#2E7D32;'>{t["longitud_m"]:.0f}</span>
            <span style='font-family:Share Tech Mono;color:#0B2545;'>{t["s_amp_mm2"]}</span>
            <span style='font-family:Share Tech Mono;color:#2E7D32;'>{t["s_cdt_mm2"]}</span>
            <span style='font-family:Share Tech Mono;color:#2E7D32;font-weight:700;font-size:0.9rem;'>{t["cal_final_mm2"]} mm²</span>
            <span style='font-family:Share Tech Mono;color:#0B2545;'>{t["amp_final_a"]:.1f}</span>
            <span style='font-family:Share Tech Mono;color:{cdt_color};font-weight:600;'>
                {t["dv_real_pct"]:.2f}% ≤{t["cdt_max_pct"]}%</span>
        </div>""", unsafe_allow_html=True)

    # ── Total caída DC
    cdt_total_color = "#2E7D32" if cdt_total_pct <= CDT_LIMITES["total"] else "#0B2545"
    st.markdown(f"""
    <div style='background:#0B2545;padding:0.5rem 0.8rem;border-radius:0 0 8px 8px;
                display:flex;justify-content:space-between;align-items:center;
                border-top:2px solid #FF9F1C;'>
        <span style='color:#2E7D32;font-size:0.8rem;'>
            ...
            <div class='sol-card' style='margin-bottom:0.6rem;'>
                ...
                <table style='width:100%;font-size:0.77rem;border-collapse:collapse;'>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;padding:0.28rem 0;'>Corriente</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{t["corriente_a"]:.2f} A</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>I diseño (×{t["factor_seg"]:.2f})</td>
                        <td style='font-family:Share Tech Mono;color:#FF9F1C;text-align:right;'>{t["i_dis"]:.2f} A</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>Longitud</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;'>{t["longitud_m"]} m</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>F. temperatura ({t["temp_amb"]}°C)</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;'>{t["ft"]:.2f}</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>F. agrupamiento ({t["n_agrup"]} circ.)</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;'>{t["fa"]:.2f}</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>S mínima por ampacidad</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{t["s_amp_mm2"]} mm²</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>S mínima por ΔV ({t["cdt_max_pct"]}%)</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;'>{t["s_cdt_raw"]:.2f} → {t["s_cdt_mm2"]} mm²</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;background:#FFFFFF;'>
                        <td style='color:#FF9F1C;font-weight:600;'>✅ Calibre seleccionado</td>
                        <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:right;font-weight:700;font-size:1rem;'>{t["cal_final_mm2"]} mm²</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>Criterio gobernante</td>
                        <td style='text-align:right;'>{_criterio_badge(t["criterio_gov"])}</td>
                    </tr>
                    <tr style='border-bottom:1px solid #0B2545;'>
                        <td style='color:#2E7D32;'>Ampacidad real</td>
                        <td style='font-family:Share Tech Mono;color:#0B2545;text-align:right;'>{t["amp_final_a"]:.1f} A</td>
                    </tr>
                    <tr>
                        <td style='color:#2E7D32;'>ΔV real</td>
    ...
    for cab, metros in sorted(resumen_cables.items()):
        mat_rows += f"""
        <tr style='border-bottom:1px solid #0B2545;'>
            <td style='color:#0B2545;padding:0.4rem 0.6rem;'>{cab}</td>
            <td style='font-family:Share Tech Mono;color:#0B2545;text-align:center;padding:0.4rem;'>{metros:.1f} m</td>
            <td style='font-family:Share Tech Mono;color:#2E7D32;text-align:center;padding:0.4rem;'>Cable FV tipo PV1-F / H07RN-F</td>
        </tr>"""

    st.markdown(f"""
    <table style='width:100%;border-collapse:collapse;font-size:0.85rem;'>
        <thead>
            <tr style='background:#0B2545;color:#2E7D32;font-size:0.75rem;text-transform:uppercase;letter-spacing:1px;'>
    ...
    <div style='font-family:Rajdhani,sans-serif;font-size:1.1rem;font-weight:700;
                color:#FF9F1C;letter-spacing:1px;margin-bottom:0.6rem;'>
