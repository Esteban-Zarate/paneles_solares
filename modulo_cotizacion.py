# modulo_cotizacion.py — EcoVolt Pro · Módulo de Cotización Comercial
# ─────────────────────────────────────────────────────────────────────────────
"""
...
def generar_pdf_cotizacion(datos: dict) -> bytes:
    ...
    COLOR_MAP = {"OFF-GRID": "#FF9F1C", "ON-GRID": "#FF9F1C", "HIBRIDO": "#FF9F1C", "HÍBRIDO": "#FF9F1C"}
    col_hex = COLOR_MAP.get(TIPO, "#FF9F1C")
    ...
    header_data = [[
        ...
        Table([[
            [p(f"<b>{emp.get('nombre','EcoVolt Pro')}</b>",
    ...
    story.append(p(
        f"ECOVOLT PRO  ·  Cotización N° {meta.get('numero','001')}  ·  "
...
def mostrar_cotizacion(proyecto_id: int, session_state: dict):
    ...
    <div class='hero-header'>
        <div class='hero-title'>💰 COTIZACIÓN COMERCIAL
            <span style='font-size:0.55em;background:#FF9F1C;color:#F8F9FA;padding:2px 12px;
    ...
    with tab_nueva:
        ...
        with col_ts2:
            ...
            <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;
                        ...
                        align-items:center;'>
                <span style='color:#2E7D32;font-size:0.82rem;'>Proyecto:
                    <b style='color:#FF9F1C;'>{dim['proyecto']}</b></span>
                <span style='color:#2E7D32;font-size:0.82rem;'>Paneles:
                    <b style='color:#0B2545;'>{dim['n_paneles']} × {dim['pot_panel_wp']}Wp</b></span>
                <span style='color:#2E7D32;font-size:0.82rem;'>Inv:
                    <b style='color:#0B2545;'>{dim['inv_kw']:.0f} kW</b></span>
                <span style='color:#2E7D32;font-size:0.82rem;'>Bat:
                    <b style='color:#0B2545;'>{dim['n_baterias']} × {dim['bat_cap_ah']}Ah</b></span>
                <span style='color:#2E7D32;font-size:0.82rem;'>VDC:
                    <b style='color:#0B2545;'>{dim['vdc']}V</b></span>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class='info-note' style='margin-bottom:0.8rem;border-color:rgba(0,230,118,0.4);border-left:4px solid #2E7D32;'>
        ...
        if dim["ben_anio_eco"] > 0:
            ...
            <div style='display:grid;grid-template-columns:repeat(4,1fr);gap:0.5rem;margin-bottom:0.8rem;'>
                <div class='metric-box' style='border-color:rgba(0,230,118,0.4);'>
                    <div class='metric-val' style='color:#2E7D32;font-size:1.1rem;'>${dim['ben_anio_eco']/1e6:.2f}M</div>
                ...
                <div class='metric-box' style='border-color:rgba(0,188,212,0.4);'>
                    <div class='metric-val' style='color:#0B2545;font-size:1.1rem;'>{dim['gen_anio_kwh']:,.0f}</div>
                ...
                <div class='metric-box' style='border-color:rgba(167,139,250,0.4);'>
                    <div class='metric-val' style='color:#2E7D32;font-size:1.1rem;'>{dim['co2_anio_eco']:,.0f}</div>
        ...
        with st.expander("✏ Editar ítems de la cotización", expanded=True):
            ...
            for _hc, _ht in zip(_hcols, ["Descripción","Cantidad","Unidad","Precio Unit.","Total"]):
                _hc.markdown(f"<div style='font-size:0.75rem;color:#FF9F1C;font-weight:700;"
            ...
            for idx, item in enumerate(items_auto):
                ...
                with cols[4]:
                    vt = cant_v * precio_v
                    st.markdown(f"""
                    <div style='background:#F8F9FA;border-radius:6px;padding:0.45rem 0.6rem;
                                font-family:Share Tech Mono,monospace;font-size:0.82rem;
                                color:#0B2545;text-align:right;margin-top:0.1rem;'>
                        $ {vt:,.0f}
                    </div>""", unsafe_allow_html=True)
            ...
            for ex_item in session_state.get("_cot_extra_items", []):
                ...
                with ex_cols[4]:
                    ...
                    st.markdown(f"""
                    <div style='background:#F8F9FA;border-radius:6px;padding:0.45rem 0.6rem;
                                font-family:Share Tech Mono;font-size:0.82rem;color:#0B2545;
        ...
        <div style='display:grid;grid-template-columns:repeat(4,1fr);gap:0.8rem;margin:0.8rem 0;'>
            ...
            <div class='metric-box' style='border-color:rgba(255,82,82,0.4);'>
                <div class='metric-val' style='color:#0B2545;'>- ${desc_val_calc:,.0f}</div>
                <div class='metric-unit'>{moneda_sym}</div>
                <div class='metric-label'>DESCUENTO {descuento_pct:.0f}%</div>
            </div>
            <div class='metric-box' style='border-color:rgba(0,188,212,0.4);'>
                <div class='metric-val' style='color:#0B2545;'>${iva_val_calc:,.0f}</div>
                <div class='metric-unit'>{moneda_sym}</div>
            ...
            <div class='metric-box' style='border-color:rgba(0,230,118,0.5);
                ...
                <div class='metric-val' style='color:#2E7D32;font-size:1.6rem;'>${total_calc:,.0f}</div>
                <div class='metric-unit'>{moneda_sym}</div>
    ...
    with tab_hist:
        ...
        if hist.empty:
            ...
            for _, row in hist.iterrows():
                estado_color = {
                    "BORRADOR":"#2E7D32","ENVIADA":"#0B2545",
                    "APROBADA":"#2E7D32","RECHAZADA":"#0B2545","VENCIDA":"#FF9F1C"
                }.get(row["estado"],"#2E7D32")
                ...
                <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;
                    ...
                    <span style='font-size:0.82rem;color:#2E7D32;'>
                        Cliente: <b style='color:#0B2545;'>{row['cliente_nombre'] or '—'}</b></span>
                    <span style='font-size:0.82rem;color:#2E7D32;'>
                        Sistema: <b style='color:#0B2545;'>{row['tipo_sistema']}</b></span>
                    <span style='font-size:0.82rem;color:#2E7D32;'>
                        Total: <b style='color:#2E7D32;font-family:Share Tech Mono;'>
            ...
            for col_h, lbl_h, val_h, col_c in [
                (c1, "Total cotizaciones", len(hist), "#FF9F1C"),
                (c2, "Aprobadas",  aprobadas, "#2E7D32"),
                (c3, "Valor total cotizado", f"${total_h:,.0f}", "#0B2545"),
                (c4, "Tasa aprobación",
                 f"{aprobadas/len(hist)*100:.0f}%", "#0B2545"),
            ]:
                col_h.markdown(f"""
