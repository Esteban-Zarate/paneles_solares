...
def generar_informe_pdf(sim: dict, proyecto_nombre: str) -> bytes:
    ...
    COLOR_MAP = {"OFF-GRID":"#FF9F1C","ON-GRID":"#FF9F1C","HIBRIDO":"#FF9F1C"}
    LABEL_MAP = {"OFF-GRID":"🔋 OFF-GRID","ON-GRID":"🔌 ON-GRID","HIBRIDO":"⚡ HÍBRIDO"}
    col_hex = COLOR_MAP.get(tipo_s, "#FF9F1C")
    ...
    BRD   = colors.HexColor("#0B2545")
    BAT   = colors.HexColor("#2E7D32")
    GRID  = colors.HexColor("#FF9F1C")
    ...
    def sec_table(title, rows):
        ...
        t.setStyle(TableStyle([
            ...
            ("ALIGN",        (0,0),(-1,0),  "LEFT"),
            *[("BACKGROUND", (0,i),(-1,i), CARD if i%2==1 else colors.HexColor("#F8F9FA"))
    ...
    # Portada
    story.append(Paragraph("☀  ECOVOLT PRO", h1))
    ...
    story.append(Spacer(1, 0.3*cm))
    story.append(Paragraph(
        f"<font color='#0B2545'>EcoVolt Pro · {lbl_pdf} · Simulación generada el "
...
def mostrar_simulador(proyecto_id: int, ss: dict):
    ...
    st.markdown("""
    <style>
    .sim-header{background:linear-gradient(135deg,#F8F9FA,#FFFFFF);border:1px solid #0B2545;
    ...
    .sim-header h2{font-family:Rajdhani,sans-serif;font-size:1.6rem;font-weight:700;
     color:#FF9F1C;margin:0;letter-spacing:2px;}
    .sim-header p{color:#2E7D32;font-size:0.8rem;margin:0.2rem 0 0;letter-spacing:2px;}
    ...
    COLOR_SIS = {"OFF-GRID": "#FF9F1C", "ON-GRID": "#FF9F1C", "HIBRIDO": "#FF9F1C"}
    ...
    col_sis  = COLOR_SIS.get(tipo_sistema, "#FF9F1C")
    lbl_sis  = LABEL_SIS.get(tipo_sistema, tipo_sistema)

    st.markdown(f"""
    <div class='sim-header'>
        <h2>🔬 SIMULADOR DE DIMENSIONAMIENTO FOTOVOLTAICO
            <span class='sis-pill' style='background:{col_sis};color:#F8F9FA;'>{lbl_sis}</span>
        </h2>
    ...
    <div style='background:rgba(0,0,0,0.2);border:1px solid {tip_color}44;
     ...
     padding:0.6rem 1rem;font-size:0.82rem;color:#2E7D32;margin-bottom:1rem;'>
        {tip_txt}
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("⚙ PARÁMETROS DE SIMULACIÓN", expanded=True):
        ...
        with colc:
            ...
            if _es_offgrid_s and _inv_modulo == 0 and _inv_presup == 0:
                st.markdown("""
                <div style='font-size:0.72rem;color:#2E7D32;margin-top:-0.5rem;margin-bottom:0.4rem;
    ...
    st.markdown("<hr style='border-color:#0B2545;margin:1.5rem 0;'>", unsafe_allow_html=True)
    st.markdown(f"""
    <div style='font-family:Rajdhani,sans-serif;font-size:1.3rem;font-weight:700;
     letter-spacing:2px;margin-bottom:1rem;'>
        ...
        <span style='font-size:0.85rem;color:#2E7D32;margin-left:1rem;'>
    ...
    for col, val, unit, sub, label in metrics_tech:
        col.markdown(f"""
        <div style='background:#FFFFFF;border:1px solid {col_sis}44;border-radius:10px;
         padding:1rem;text-align:center;'>
            ...
            <div style='font-size:0.72rem;color:#0B2545;margin-top:0.2rem;'>{unit}</div>
            <div style='font-size:0.7rem;color:#2E7D32;margin-top:0.3rem;
             text-transform:uppercase;letter-spacing:1px;'>{label}</div>
            <div style='font-size:0.7rem;color:#0B2545;margin-top:0.2rem;'>{sub}</div>
        </div>""", unsafe_allow_html=True)
    ...
    _rows_offgrid = (
        f"<tr><td style='color:#2E7D32;'>Tensión DC</td>"
        f"<td style='color:#0B2545;text-align:right;font-family:Share Tech Mono,monospace;'>{vdc_s} V</td></tr>"
        f"<tr><td style='color:#2E7D32;'>Energía banco bat.</td>"
        f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{bats_s['energia_kwh']:.2f} kWh</td></tr>"
        f"<tr><td style='color:#2E7D32;'>Cobertura solar</td>"
        f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>100% (aislado)</td></tr>"
    )
    _rows_ongrid = (
        f"<tr><td style='color:#2E7D32;'>Autoconsumo/día</td>"
        f"<td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>{_ac:.2f} kWh</td></tr>"
        f"<tr><td style='color:#2E7D32;'>Inyección red/día</td>"
        f"<td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>{_inj:.2f} kWh</td></tr>"
        f"<tr><td style='color:#2E7D32;'>Cobertura solar</td>"
        f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{_cob:.0f}%</td></tr>"
    )
    _rows_hibrido = (
        f"<tr><td style='color:#2E7D32;'>Autoconsumo/día</td>"
        f"<td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>{_ac:.2f} kWh</td></tr>"
        ...
        f"<tr><td style='color:#2E7D32;'>Cobertura solar</td>"
        f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{_cob:.0f}%</td></tr>"
        f"<tr><td style='color:#2E7D32;'>Banco baterías</td>"
        f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{bats_s['energia_kwh']:.2f} kWh</td></tr>"
        f"<tr><td style='color:#2E7D32;'>Autonomía batería</td>"
        f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{autonomia_s:.1f} h</td></tr>"
    )
    ...
    _row_iny_on = (
        f"<tr><td style='color:#2E7D32;'>Ahorro autoconsumo/mes</td>"
        f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>$ {_autocon_mes:,.0f}</td></tr>"
        f"<tr><td style='color:#2E7D32;'>Ingreso inyección/mes</td>"
        f"<td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>$ {_iny_mes:,.0f}</td></tr>"
    )
    ...
    with col_en:
        ...
        st.markdown(f"""
        <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;padding:1.2rem;'>
            ...
            <table style='width:100%;font-size:0.8rem;'>
                <tr><td style='color:#2E7D32;'>Consumo base</td>
                    <td style='color:#0B2545;text-align:right;font-family:Share Tech Mono,monospace;'>{consumo_sim:,.0f} Wh/día</td></tr>
                <tr><td style='color:#2E7D32;'>Con FS ({fs_label})</td>
                    <td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>{consumo_fs_s:,.0f} Wh/día</td></tr>
                <tr><td style='color:#2E7D32;'>HSP bruta</td>
                    <td style='color:#0B2545;text-align:right;font-family:Share Tech Mono,monospace;'>{hsp_sim} h/día</td></tr>
                <tr><td style='color:#2E7D32;'>PR aplicado</td>
                    <td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>{pr_sim}%</td></tr>
                <tr><td style='color:#2E7D32;'>HSP efectiva</td>
                    <td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>{hsp_ef_s:.2f} h/día</td></tr>
                {rows_extra}
                <tr><td style='color:#2E7D32;'>Generación estimada</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{gen_dia_s:.2f} kWh/día</td></tr>
                <tr><td style='color:#2E7D32;'>Generación anual</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{gen_anual_s:,.0f} kWh/año</td></tr>
            </table>
        </div>""", unsafe_allow_html=True)

    with col_fi:
        vpn_color = "#2E7D32" if vpn_s > 0 else "#0B2545"
        # Flags de sistema (ya definidos globalmente al inicio)
        vpn_color = "#2E7D32" if vpn_s > 0 else "#0B2545"

        # Fila precio inyección (solo red)
        _precio_iny_row = (
            f"<tr><td style='color:#2E7D32;'>Precio inyección kWh</td>"
            f"<td style='color:#0B2545;text-align:right;font-family:Share Tech Mono,monospace;'>"
        ...
        _row_desglose = (
            f"<tr><td style='color:#2E7D32;'>Ahorro autoconsumo/mes</td>"
            f"<td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>"
            f"$ {_autocon_mes:,.0f}</td></tr>"
            f"<tr><td style='color:#2E7D32;'>Ingreso inyección/mes</td>"
            f"<td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>"
            f"$ {_iny_mes:,.0f}</td></tr>"
        ) if _es_red_s else ""
        ...
        <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;padding:1.2rem;'>
            ...
            <table style='width:100%;font-size:0.8rem;'>
                <tr><td style='color:#2E7D32;'>Costo sistema</td>
                    <td style='color:#0B2545;text-align:right;font-family:Share Tech Mono,monospace;'>$ {costo_s:,.0f}</td></tr>
                <tr><td style='color:#2E7D32;'>Tarifa kWh</td>
                    <td style='color:#0B2545;text-align:right;font-family:Share Tech Mono,monospace;'>$ {tarifa_s:,.0f}</td></tr>
                {_precio_iny_row}
                {_row_desglose}
                <tr><td style='color:#2E7D32;'>{_lbl_ben}</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>$ {_val_ben:,.0f}</td></tr>
                <tr><td style='color:#2E7D32;'>{_lbl_ben_anio}</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>$ {_val_ben_anio:,.0f}</td></tr>
                <tr><td style='color:#2E7D32;'>Payback</td>
                    <td style='color:#FF9F1C;text-align:right;font-family:Share Tech Mono,monospace;'>{pb_s:.1f} años</td></tr>
                <tr><td style='color:#2E7D32;'>TIR ({int(anos_s)} años)</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{tir_s:.1f}%</td></tr>
                <tr><td style='color:#2E7D32;'>VPN ({int(anos_s)} años)</td>
    ...
    with col_am:
        ...
        <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:10px;padding:1.2rem;'>
            ...
            <table style='width:100%;font-size:0.8rem;'>
                <tr><td style='color:#2E7D32;'>CO₂ evitado/año</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{co2_anual_s:,.0f} kg</td></tr>
                <tr><td style='color:#2E7D32;'>CO₂ evitado/25 años</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{co2_anual_s*25/1000:.1f} ton</td></tr>
                <tr><td style='color:#2E7D32;'>Árboles equivalentes</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{co2_anual_s/21:.0f} árboles/año</td></tr>
                <tr><td style='color:#2E7D32;'>Generación anual</td>
                    <td style='color:#0B2545;text-align:right;font-family:Share Tech Mono,monospace;'>{gen_anual_s:,.0f} kWh/año</td></tr>
                <tr><td style='color:#2E7D32;'>Factor CO₂ usado</td>
                    <td style='color:#2E7D32;text-align:right;font-family:Share Tech Mono,monospace;'>{factor_co2:.3f} kg/kWh</td></tr>
            </table>
        </div>""", unsafe_allow_html=True)
    ...
    if not sims_hist.empty:
        ...
        <div style='font-family:Rajdhani,sans-serif;font-size:1rem;font-weight:600;
         color:#FF9F1C;letter-spacing:1px;margin-bottom:0.5rem;'>📋 HISTORIAL DE SIMULACIONES</div>
        """, unsafe_allow_html=True)
