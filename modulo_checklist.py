...
def _chk_items(items, saved, prefix):
    ...
    for item in items:
        ...
        cols[0].markdown(f"<div style='font-size:0.83rem;color:#0B2545;padding-top:0.45rem;'>{item}</div>",
...
def _sec_header(icon, num, title):
    st.markdown(f"""
    <div style='background:linear-gradient(90deg,#0B2545,#FFFFFF);
                border-left:3px solid #FF9F1C;border-radius:0 8px 8px 0;
                padding:0.5rem 1rem;margin:0.8rem 0 0.4rem 0;
                font-family:Rajdhani,sans-serif;font-weight:600;font-size:1rem;color:#FF9F1C;'>
        {icon} {num}. {title}
    </div>""", unsafe_allow_html=True)
    st.markdown(
        "<div style='display:grid;grid-template-columns:3fr 0.6fr 0.6fr 2.5fr;"
        "gap:4px;font-size:0.75rem;color:#2E7D32;padding:0 0 0.2rem 0;'>"
...
def _chk_bool(label, saved, key, prefix):
    ...
    c1.markdown(f"<div style='font-size:0.83rem;color:#0B2545;padding-top:0.45rem;'>{label}</div>",
                unsafe_allow_html=True)
...
def generar_pdf_checklist(datos: dict) -> bytes:
    ...
    WHITE = colors.white
    LGRN  = colors.HexColor("#F8F9FA")
    LRED  = colors.HexColor("#F8F9FA")
    YELL  = colors.HexColor("#F8F9FA")
    ...
    story += [t_port, sp(0.4),
              ...
              p(f"ECOVOLT PRO — Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}", st_foot),
              pb()]
    ...
    story.append(p(f"ECOVOLT PRO | Checklist Puesta en Marcha FV | "
...
def mostrar_checklist(proyecto_id: int, session_state: dict) -> None:
    ...
    with tabs[15]:
        ...
        for lbl, key in items_r:
            ...
            c1.markdown(f"<div style='font-size:0.83rem;color:#0B2545;padding-top:0.45rem;'>{lbl}</div>",
                         unsafe_allow_html=True)
