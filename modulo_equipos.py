...
CATALOGO_EQUIPOS = [
    ...
    ("Herramienta",  "Informática",         "Laptop con software EcoVolt/PVSyst",            "DÍA",   80000, "Simulación y documentación"),
...
def mostrar_personal():
    ...
    <div style='background:rgba(255,82,82,0.08);border:1px solid rgba(255,82,82,0.25);
     border-radius:8px;padding:0.7rem 1rem;font-size:0.8rem;color:#2E7D32;margin-bottom:1rem;'>
    ...
    if not df_p.empty:
        for _, pr in df_p.iterrows():
            retie_badge = "<span style='background:#0B2545;color:#fff;padding:1px 6px;border-radius:4px;font-size:0.7rem;'>RETIE</span>" if pr["retie"] else ""
            st.markdown(f"""
             ...
             padding:1rem 1.2rem;margin-bottom:0.7rem;'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div style='font-family:Rajdhani,sans-serif;font-size:1rem;font-weight:700;
                     color:#FF9F1C;'>{pr['cargo']} {retie_badge}</div>
                    <div style='font-family:Share Tech Mono,monospace;color:#2E7D32;font-size:0.95rem;'>
                        $ {pr['salario_dia']:,.0f} / día</div>
                </div>
                <div style='font-size:0.82rem;color:#2E7D32;margin-top:0.4rem;line-height:1.6;'>
                    <b style='color:#0B2545;'>Perfil:</b> {pr['perfil']}</div>
