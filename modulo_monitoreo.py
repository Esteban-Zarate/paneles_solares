...
EcoVolt Pro · Módulo externo (solo administradores)
...
SOL   = "#FF9F1C"; GREEN = "#2E7D32"; RED = "#0B2545"; CYAN = "#0B2545"
YEL   = "#0B2545"; TEXT2 = "#2E7D32"; BRD  = "#0B2545"; CARD = "#FFFFFF"
    ...
    if row and row[0]:
        ...
        <div style='max-width:480px;margin:15vh auto;text-align:center;
            ...
            <div style='color:{TEXT2};margin-top:0.6rem;font-size:0.9rem;'>
                Un administrador cerró tu sesión en EcoVolt Pro.<br>
...
def mostrar_monitoreo(usuario_activo_fn=None, tiene_permiso_fn=None,
    ...
    st.markdown(f"""
    <div style='background:linear-gradient(135deg,#F8F9FA,#FFFFFF);border:1px solid {BRD};
    ...
    with mt1:
        ...
        if df.empty:
            ...
            st.markdown("<hr class='sep'>" if False else
                         "<hr style='border-color:#0B2545;margin:1rem 0;'>",
                         unsafe_allow_html=True)
            ...
            for _, r in df.iterrows():
                ...
                with cA:
                    ...
                    <div style='padding:0.3rem 0;'>
                        ...
                        <span style='color:#2E7D32;font-size:0.75rem;'> · {r['rol'] or '—'}</span>
                ...
                with cD:
                    if r["kicked"]:
                        st.markdown(f"""<div style='font-size:0.75rem;color:{RED};'>
                            🔌 Cerrando sesión…<br>
                            <span style='color:#2E7D32;'>desde {r['login_time']}</span>
                        </div>""", unsafe_allow_html=True)
                    else:
                        st.markdown(f"""<div style='font-size:0.75rem;color:{col};'>
                            {estado}<br><span style='color:#2E7D32;'>desde {r['login_time']}</span>
                        </div>""", unsafe_allow_html=True)
                with cF:
                ...
                with cE:
                    if es_propia:
                        ...
                        if st.button("🔌 Desconectar", key=f"kick_{r['session_id']}",
                            ...
                            st.rerun()
                st.markdown("<hr style='border-color:#F8F9FA;margin:0.2rem 0;'>",
                            unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style='margin-top:1rem;font-size:0.72rem;color:#2E7D32;text-align:center;'>
