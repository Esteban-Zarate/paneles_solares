# modulo_seguridad.py — EcoVolt Pro · Módulo de Seguridad y Control de Acceso
# ═══════════════════════════════════════════════════════════════════════════════
"""
Gestión completa de usuarios, roles y permisos para EcoVolt Pro.
...
def init_seguridad_db():
    ...
    # La contraseña inicial se obtiene de .streamlit/secrets.toml; nunca se
    # deja escrita en el código fuente. Después queda almacenada como hash en SQLite.
    existing = c.execute("SELECT COUNT(*) FROM usuarios").fetchone()[0]
    if existing == 0:
        pwd_hash = _hash_password(_get_secret_password("admin_password"))
        c.execute("""
            INSERT INTO usuarios (username, nombre_completo, email, password_hash, rol)
            VALUES (?, ?, ?, ?, ?)
        """, ("admin", "Esteban Zarate", "zarateyesesteban2009@gmail.com", pwd_hash, "superadmin"))
...
# ═══════════════════════════════════════════════════════════════════════════════
def _get_secret_password(key: str) -> str:
    """Lee una contraseña inicial desde Streamlit Secrets.

    Las contraseñas de uso normal se almacenan únicamente como hash en SQLite.
    El secreto solo se usa como valor inicial cuando la base de datos está vacía.
    """
    try:
        auth = st.secrets.get("auth", {})
        value = auth.get(key)
        if not value:
            raise RuntimeError(f"Falta el secreto auth.{key} en .streamlit/secrets.toml")
        return str(value)
    except Exception as exc:
        raise RuntimeError(f"No se pudo leer el secreto auth.{key}: {exc}") from exc


def _hash_password(password: str) -> str:
    """SHA-256 con sal fija de la app."""
    salt = "EcoVoltPro_2024_#SEC"
...
def mostrar_login():
    ...
    .login-card{background:#FFFFFF;border:1px solid #0B2545;border-radius:16px;
     padding:2.5rem 3rem;max-width:420px;width:100%;}
    .login-logo{text-align:center;margin-bottom:1.5rem;}
    .login-logo .brand{font-family:Rajdhani,sans-serif;font-size:2.6rem;
     font-weight:700;color:#FF9F1C;letter-spacing:3px;line-height:1;}
    .login-logo .sub{font-size:0.65rem;color:#2E7D32;letter-spacing:4px;margin-top:0.3rem;}
    .login-divider{border:none;border-top:1px solid #0B2545;margin:1.2rem 0;}
    </style>
    ...
    with col_c:
        ...
        <div class='login-logo'>
            <div class='brand'>☀ ESTEBAN<br>ZARATE</div>
        ...
        with st.container():
            st.markdown("""
            <div style='background:#FFFFFF;border:1px solid #0B2545;border-radius:16px;
            ...
            <div style='text-align:center;margin-bottom:1.2rem;'>
                <span style='font-family:Rajdhani,sans-serif;font-size:1.1rem;
                 color:#FF9F1C;font-weight:600;letter-spacing:2px;'>
                    🔐 INICIAR SESIÓN
                </span>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style='text-align:center;margin-top:1.2rem;font-size:0.75rem;
             color:#0B2545;letter-spacing:1px;'>
                EcoVolt Pro · Acceso restringido<br>
...
def mostrar_usuario_sidebar():
    ...
    st.markdown(f"""
    <div style='background:#FFFFFF;border:1px solid #B8DDE0;border-radius:10px;
     padding:0.7rem 0.9rem;margin-bottom:0.5rem;'>
        <div style='font-family:Rajdhani,sans-serif;font-size:0.95rem;
         color:#123047;font-weight:600;'>{rol_info["icon"]} {u["nombre"]}</div>
        <div style='font-size:0.7rem;color:#2E9D5B;margin-top:0.2rem;'>
            @{u["username"]}
        </div>
...
def mostrar_gestion_usuarios():
    ...
    with tab_lista:
        ...
        <div style='display:flex;gap:1rem;margin-bottom:1rem;flex-wrap:wrap;'>
            <div style='background:#FFFFFF;border-radius:8px;padding:0.6rem 1.2rem;text-align:center;'>
                <div style='font-family:Share Tech Mono;font-size:1.4rem;color:#FF9F1C;'>{len(usuarios_df)}</div>
                <div style='font-size:0.72rem;color:#2E7D32;'>Usuarios totales</div>
            </div>
            <div style='background:#FFFFFF;border-radius:8px;padding:0.6rem 1.2rem;text-align:center;'>
                <div style='font-family:Share Tech Mono;font-size:1.4rem;color:#2E7D32;'>{usuarios_df["activo"].sum()}</div>
                <div style='font-size:0.72rem;color:#2E7D32;'>Activos</div>
            </div>
            <div style='background:#FFFFFF;border-radius:8px;padding:0.6rem 1.2rem;text-align:center;'>
                <div style='font-family:Share Tech Mono;font-size:1.4rem;color:#0B2545;'>{(~usuarios_df["activo"].astype(bool)).sum()}</div>
                <div style='font-size:0.72rem;color:#2E7D32;'>Inactivos</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
            ...
            with st.expander(
                ...
                with col1:
                    ...
                    <table style='font-size:0.82rem;width:100%;border-collapse:collapse;'>
                        <tr><td style='color:#2E7D32;padding:0.25rem 0;'>ID</td>
                            <td style='font-family:Share Tech Mono;color:#0B2545;'>#{row['id']}</td></tr>
                        <tr><td style='color:#2E7D32;padding:0.25rem 0;'>Usuario</td>
                            <td style='color:#0B2545;'>@{row['username']}</td></tr>
                        <tr><td style='color:#2E7D32;padding:0.25rem 0;'>Correo</td>
                            <td style='color:#0B2545;'>{row['email']}</td></tr>
                        <tr><td style='color:#2E7D32;padding:0.25rem 0;'>Rol</td>
                            <td style='color:{rol_i["color"]};font-weight:700;'>{rol_i["label"]}</td></tr>
                        <tr><td style='color:#2E7D32;padding:0.25rem 0;'>Creado</td>
                            <td style='font-family:Share Tech Mono;color:#2E7D32;'>{row['creado'][:16]}</td></tr>
                        <tr><td style='color:#2E7D32;padding:0.25rem 0;'>Último acceso</td>
                            <td style='font-family:Share Tech Mono;color:#2E7D32;'>{str(row['ultimo_acceso'] or '—')[:16]}</td></tr>
                    </table>
                    """, unsafe_allow_html=True)

                with col2:
                    st.markdown("<div style='color:#2E7D32;font-size:0.75rem;margin-bottom:0.4rem;'>PERMISOS DEL ROL</div>", unsafe_allow_html=True)
                    perms = ROLES.get(row["rol"], {}).get("permisos", [])
                    for p in PERMISOS_LABELS:
                        tiene = p in perms
                        st.markdown(
                            f"<div style='font-size:0.75rem;color:{'#2E7D32' if tiene else '#0B2545'};'>"
                            f"{'✓' if tiene else '✗'} {PERMISOS_LABELS[p]}</div>",
                            unsafe_allow_html=True)

                with col3:
                    if tiene_permiso("editar_usuarios") and row["id"] != u["id"]:
                        st.markdown("<div style='color:#2E7D32;font-size:0.75rem;margin-bottom:0.4rem;'>ACCIONES</div>", unsafe_allow_html=True)
                        ...
                        if tiene_permiso("eliminar_usuarios") and row["rol"] != "superadmin":
                            if st.checkbox("⚠ Eliminar usuario", key=f"chk_del_{row['id']}"):
                                st.markdown("<div style='color:#0B2545;font-size:0.75rem;'>Esta acción es permanente.</div>", unsafe_allow_html=True)
    ...
    with tab_auditoria:
        if not tiene_permiso("ver_auditoria"):
            ...
            st.markdown(f"<div style='color:#2E7D32;font-size:0.8rem;margin-bottom:0.5rem;'>"
                        f"Mostrando {len(df_show)} de {len(aud_df)} registros</div>",
                        unsafe_allow_html=True)

            for _, row in df_show.head(100).iterrows():
                accion_col = {"LOGIN": "#2E7D32", "LOGOUT": "#2E7D32",
                              "CREAR": "#0B2545", "ELIMINAR": "#0B2545",
                              "CAMBIO": "#FF9F1C"}.get(row["accion"][:6], "#2E7D32")
                st.markdown(f"""
                <div style='background:#FFFFFF;border-left:3px solid {accion_col};
                 border-radius:6px;padding:0.4rem 0.8rem;margin-bottom:0.3rem;
                 display:flex;gap:1rem;align-items:center;font-size:0.78rem;'>
                    <span style='color:#0B2545;font-family:Share Tech Mono;min-width:130px;'>{str(row['fecha'])[:16]}</span>
                    <span style='color:#FF9F1C;font-weight:600;min-width:100px;'>@{row['username'] or '—'}</span>
                    <span style='color:{accion_col};font-family:Share Tech Mono;min-width:140px;'>{row['accion']}</span>
                    <span style='color:#2E7D32;min-width:90px;'>{row['modulo'] or ''}</span>
                    <span style='color:#0B2545;'>{row['detalle'] or ''}</span>
                </div>
                """, unsafe_allow_html=True)

            if tiene_permiso("gestion_sistema"):
                st.markdown("<hr style='border-color:#0B2545;margin:1rem 0;'>", unsafe_allow_html=True)
...
def mostrar_cambio_password():
    ...
    with col_p2:
        ...
        <div class='sol-card'>
            ...
            <table style='font-size:0.82rem;width:100%;border-collapse:collapse;'>
                <tr><td style='color:#2E7D32;padding:0.3rem 0;'>ID</td>
                    <td style='font-family:Share Tech Mono;color:#0B2545;'>#{u['id']}</td></tr>
                <tr><td style='color:#2E7D32;padding:0.3rem 0;'>Nombre</td>
                    <td style='color:#0B2545;'>{u['nombre']}</td></tr>
                <tr><td style='color:#2E7D32;padding:0.3rem 0;'>Usuario</td>
                    <td style='color:#0B2545;'>@{u['username']}</td></tr>
                <tr><td style='color:#2E7D32;padding:0.3rem 0;'>Correo</td>
                    <td style='color:#0B2545;'>{u['email']}</td></tr>
                <tr><td style='color:#2E7D32;padding:0.3rem 0;'>Rol</td>
                    <td style='color:{ROLES[u["rol"]]["color"]};font-weight:700;'>
                        {ROLES[u["rol"]]["icon"]} {ROLES[u["rol"]]["label"]}</td></tr>
            </table>
            <div style='margin-top:1rem;'>
                <div style='color:#2E7D32;font-size:0.75rem;margin-bottom:0.4rem;'>MIS PERMISOS</div>
                {''.join(
                    f"<div style='font-size:0.73rem;color:#2E7D32;'>✓ {PERMISOS_LABELS.get(p,p)}</div>"
