...
YEL   = "#0B2545"; PUR   = "#2E7D32"; TEXT2 = "#2E7D32"; BRD = "#0B2545"
CARD  = "#FFFFFF"
        ...
        with col:
            ...
            for _, r in sub.iterrows():
                ...
                <div class='sol-card' style='padding:0.7rem 0.8rem;margin-bottom:0.5rem;
                    ...
                    <div style='font-weight:600;font-size:0.85rem;color:#0B2545;'>{r['nombre']}</div>
        ...
        <div style='display:flex;justify-content:space-between;align-items:center;
            ...
            <div>
                <div style='font-family:Rajdhani,sans-serif;font-size:1.4rem;
                            font-weight:700;color:#0B2545;'>{c['nombre']}</div>
    ...
    with dtab6:
        ...
        if not c["email"]:
            ...
            with st.form(f"form_email_{cliente_id}"):
                asunto = st.text_input("Asunto", value=f"EcoVolt Pro — {c['nombre']}",
                                        key=f"asunto_email_{cliente_id}")
                cuerpo = st.text_area(
                    "Mensaje", height=140,
                    value=f"Hola {c['nombre']},\n\nTe escribo desde EcoVolt Pro "
...
def _mostrar_exportar_importar(df_clientes, usuario, es_admin, registrar_auditoria_fn):
    ...
    with st.form("form_email_export"):
        ...
        asunto_export = st.text_input("Asunto", value=f"CRM EcoVolt Pro — Exportación {_hoy()}")
    ...
    if enviar_export:
        if not dest_export.strip():
            ...
            ok, error = enviar_email(
                ...
                cuerpo="Adjunto la exportación del CRM de EcoVolt Pro.",
